...
class ConfigurationManager {
    ...
    async configureBasicSettings(): Promise<void> {
        console.log('\n=== Basic Settings ===');
    ...
    async configureTimeout(): Promise<void> {
    ...
    async configureSounds(): Promise<void> {
        // Load desktop channel to get available sounds
        const DesktopChannel = require('../channels/local/desktop') as any;
        const desktop = new DesktopChannel();
        const soundCategories: Record<string, string[]> = desktop.getAvailableSounds();
    ...
    async selectSoundFromCategories(soundCategories: Record<string, string[]>, type: string): Promise<string | null> {
    ...
    async selectSoundFromList(sounds: string[], type: string): Promise<string | null> {
        console.log(`\nSelect ${type} sound:`);
    ...
    async configureCustomMessages(): Promise<void> {
    ...
    async testNotifications(): Promise<void> {
        ...
        try {
            const Notifier = require('../core/notifier') as any;
    ...
    async configureEmailChannel(): Promise<void> {
        console.log('\n=== Email Notification Configuration ===');

        // Get current email configuration
        const currentEmailConfig = this.config.getChannel('email') || { enabled: false, config: {} } as any;
        const emailConfig: Record<string, any> = currentEmailConfig.config || {};
        ...
        // Save configuration
        this.config.setChannel('email', newEmailConfig as any);
    ...
    async testEmailChannel(): Promise<void> {
        console.log('\n🧪 Testing email sending...');

        try {
            const EmailChannel = require('../channels/email/smtp') as any;
            ...
            console.log('❌ Email test failed:', (error as Error).message);
