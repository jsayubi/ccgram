...
const Imap = optionalRequire('node-imap', 'IMAP email listening') as any;
const mailparserModule = optionalRequire('mailparser', 'email parsing') as any;
const simpleParser: ((source: string) => Promise<any>) | null = mailparserModule ? mailparserModule.simpleParser : null;
...
interface SentMessagesData {
    messages: Array<{ messageId: string; sentAt: string }>;
}
...
class EmailListener extends EventEmitter {
    ...
    async start(): Promise<void> {
        ...
        try {
            ...
            this.logger.error('Failed to start email listener:', (error as Error).message);
            throw error;
        }
    }
    ...
    async _checkNewMails(): Promise<void> {
        try {
            await this._openInbox();
            await this._searchAndProcessMails();
        } catch (error: unknown) {
            this.logger.error('Error checking emails:', (error as Error).message);
            ...
            if (!this.isConnected) {
                ...
                try {
                    ...
                    this.logger.error('Reconnection failed:', (reconnectError as Error).message);
                }
            }
        }
    }

    async _openInbox(): Promise<any> {
        return new Promise((resolve, reject) => {
            this.imap.openBox('INBOX', false, (error: Error | null, box: any) => {
                if (error) {
                    reject(error);
                } else {
                    resolve(box);
    ...
    async _searchAndProcessMails(): Promise<void> {
        return new Promise((resolve, reject) => {
    ...
    _processMessage(msg: any, seqno: number): void {
        let buffer = '';
        ...
        msg.on('body', (stream: any, info: any) => {
            ...
            stream.once('end', async () => {
                try {
                    ...
                    this.logger.error('Email parsing error:', (parseError as Error).message);
                }
            });
        });
    ...
    async _handleParsedEmail(email: any, seqno: number): Promise<void> {
        try {
            ...
            this.logger.error(`Error handling email ${seqno}:`, (error as Error).message);
        }
    }

    _isReplyEmail(email: any): boolean {
    ...
    _extractSessionId(email: any): string | null {
    ...
    _getSessionIdByToken(token: string): string | null {
        ...
        try {
            ...
            this.logger.error('Error looking up session by token:', (error as Error).message);
        }
        return null;
    }

    async _validateSession(sessionId: string): Promise<SessionData | null> {
        ...
        try {
            ...
            this.logger.error(`Error reading session ${sessionId}:`, (error as Error).message);
            return null;
        }
    }

    _extractCommand(email: any): string {
        let text: string = email.text || '';
    ...
    _cleanEmailContent(text: string): string {
    ...
    _isCommandSafe(command: string): boolean {
    ...
    async updateSessionCommandCount(sessionId: string): Promise<void> {
        const sessionFile = path.join(this.sessionsDir, `${sessionId}.json`);
        ...
        if (fs.existsSync(sessionFile)) {
            try {
                ...
                this.logger.error(`Error updating session ${sessionId}:`, (error as Error).message);
            }
        }
    }

    async _isSystemSentEmail(messageId: string): Promise<boolean> {
        if (!messageId || !fs.existsSync(this.sentMessagesPath)) {
            return false;
        }

        try {
            const sentMessages: SentMessagesData = JSON.parse(fs.readFileSync(this.sentMessagesPath, 'utf8'));
            return sentMessages.messages.some(msg => msg.messageId === messageId);
        } catch (error: unknown) {
            this.logger.error('Error reading sent messages:', (error as Error).message);
            return false;
        }
    }

    async _removeFromSentMessages(messageId: string): Promise<void> {
        ...
        try {
            ...
            this.logger.error('Error removing from sent messages:', (error as Error).message);
        }
    }
}
