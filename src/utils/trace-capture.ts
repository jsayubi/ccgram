...
class TraceCapture {
    ...
    constructor() {
        this.dataDir = path.join(PROJECT_ROOT, 'src/data');
    ...
    _loadTimestamps(): TimestampsMap {
        try {
            ...
            console.error('Failed to load timestamps:', (error as Error).message);
        }
        return {};
    }
    ...
    _saveTimestamps(data: TimestampsMap): void {
        try {
            ...
            console.error('Failed to save timestamps:', (error as Error).message);
        }
    }
     ...
     */
    recordUserInput(sessionName: string, timestamp: number = Date.now()): void {
