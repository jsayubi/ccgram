...
import { optionalRequire, getUUID } from '../../utils/optional-require';
const nodemailer = optionalRequire('nodemailer', 'email notifications') as any;
...
class EmailChannel extends NotificationChannel {
    ...
    _initializeTransporter(): void {
        ...
        try {
            this.transporter = nodemailer.createTransport({
                ...
                connectionTimeout: parseInt(process.env.SMTP_TIMEOUT as string) || 10000,
                greetingTimeout: parseInt(process.env.SMTP_TIMEOUT as string) || 10000,
                socketTimeout: parseInt(process.env.SMTP_TIMEOUT as string) || 10000
            });
    ...
    async _createSession(sessionId: string, notification: Notification, token: string): Promise<void> {
        ...
        const tmuxSession = (notification.metadata?.tmuxSession as string) || this._getCurrentTmuxSession() || 'ccgram';
    ...
    async _trackSentMessage(messageId: string, sessionId: string, token: string): Promise<void> {
        let sentMessages: SentMessages = { messages: [] };
    ...
    _generateEmailContent(notification: Notification, sessionId: string, token: string): EmailContent {
        ...
        if (notification.metadata) {
            userQuestion = (notification.metadata.userQuestion as string) || '';
            claudeResponse = (notification.metadata.claudeResponse as string) || '';
        }
...
==================================`;
        ...
        const variables: Record<string, string> = {
            ...
            subagentActivities: (notification.metadata?.subagentActivities as string) || '',
            executionTraceSection: executionTraceSection,
            executionTraceText: executionTraceText,
            fullExecutionTrace: (notification.metadata?.fullExecutionTrace as string) ||
...
The trace provides complete transparency about what happened during task execution.`
            ...
            this.logger.error('Email test failed:', (error as Error).message);
            return false;
        }
    }
